// Run: NODE_OPTIONS=--conditions=react-server npx tsx scripts/backfill-margin-alerts.ts
...
async function main() {
  ...
  try {
    ...
    for (const workspace of workspaces) {
      await evaluateAllMarginAlerts(workspace.id, db);
      evaluated += 1;
      console.log(`Evaluated margin alerts for workspace ${workspace.id}`);
    }
